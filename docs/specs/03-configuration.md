...
- *Interactive SSO login from kleya.* SSO **token consumption** works today via `aws-config`'s default-enabled `sso` feature, but interactive login requires a separate `aws sso login` invocation from the AWS CLI v2. Draft design at [`../superpowers/specs/2026-05-19-sso-login-design.md`](../superpowers/specs/2026-05-19-sso-login-design.md) proposes enabling the `credentials-login` Cargo feature plus a `kleya sso login/logout/status` subcommand tree to remove the AWS CLI v2 dependency. No `[sso]` config section is proposed — the design leans on the existing `~/.aws/config` profile fields.
