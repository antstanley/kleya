...
| [11-credentials-and-sso.md](11-credentials-and-sso.md) | AWS credentials chain, profile / region resolution, SSO + `aws login` console-credentials via cached tokens, why kleya never owns login |
...
- The operator has working AWS credentials reachable through `aws-config`'s default chain (env vars, profile, SSO cached token, `aws login` console credentials via the `credential_process` shim, web-identity OIDC, or IMDS). Authentication itself happens outside kleya — `aws sso login`, `aws login`, `aws configure`, `aws-vault`, `granted`, etc. See [11-credentials-and-sso.md](11-credentials-and-sso.md).
