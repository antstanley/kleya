...
`kleya` does not manage AWS credentials itself. Every adapter call delegates to `aws-config`'s default credentials chain, which resolves to one of the supported sources at runtime: environment variables, an `~/.aws/config` profile, an IAM Identity Center (SSO) cached token, an `aws login` console-credentials session (via the `credential_process` shim), a `credential_process` helper, instance-profile metadata (IMDS), or an OIDC web-identity token. This page documents which sources are reachable in the current code, where each one is read from, how kleya's profile / region overrides interact with the SDK's own resolution, and how a credential failure surfaces.
...
| **Console credentials (`aws login`)** | `~/.aws/login/cache/<profile>` written by `aws login`; profile block in `~/.aws/config` has `login_session = arn:aws:iam::…:user/...` | Run `aws login --profile <p>` against an IAM user / root / federated identity (AWS CLI v2 ≥ 2.32.0). **Requires a `credential_process` sibling-profile shim** until `aws-config` adds native `login_session` support — see "Browser-based login" below. |
...
The cache lives at `~/.aws/login/cache/<profile>` (override via `AWS_LOGIN_CACHE_DIRECTORY`). The profile block in `~/.aws/config` carries a `login_session = arn:aws:iam::<account>:user/<name>` line that identifies the session.

`aws-config 1.8.x` does **not** yet read the `login_session` profile key natively. Until it does, the supported integration path is the [credential-process shim documented in the AWS CLI guide](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-sign-in.html#cli-configure-sign-in-cached-credentials):
...
Operator then runs `aws login --profile signin` once per session, and points kleya at the sibling profile: `kleya launch --profile kleya` (or `AWS_PROFILE=kleya`). The `credentials-process` provider in `aws-config`'s default chain invokes the AWS CLI on demand and consumes the credentials it exports.

When `aws-config` adds native `login_session` support, the shim becomes unnecessary and kleya picks the flow up transparently — no kleya code change required. Tracked in Open Questions.
...
- `~/.aws/login/cache/<profile>` — only indirectly, via the `aws configure export-credentials` shim invoked by a `credential_process` profile (see "Browser-based login" above). `aws-config` does not read this directory directly today.
...
- *Authentication happens outside kleya.* **The CLI never owns a login, logout, or auth-driving subcommand.** Operators authenticate using the AWS CLI v2 (`aws sso login` for IAM Identity Center, `aws login` for console credentials, `aws configure` for static keys), `aws-vault`, `granted`, or any equivalent tool of their choice; kleya consumes whatever credentials the SDK's default chain finds. This rule applies to future providers too — when a non-AWS adapter is added, its login flow stays in the provider's own tooling. The `aws-config` Cargo feature `credentials-login` (which bundles `aws-sdk-signin` for driving interactive login) is therefore deliberately *not* enabled.
...
- *Native `login_session` support in `aws-config`.* The AWS CLI v2 ≥ 2.32.0 `aws login` command writes credentials to `~/.aws/login/cache/<profile>` and a `login_session = …` line in `~/.aws/config`. The Rust `aws-config 1.8.x` default chain does not yet read the `login_session` key; operators must install the documented `credential_process` shim (`aws configure export-credentials --profile <signin> --format process`) to feed those credentials into kleya. When `aws-config` adds native support (likely a future minor version), the shim becomes unnecessary and operators can point kleya directly at the login-session profile. No kleya code change is required either way — watch for the SDK release and update this page when it lands.
