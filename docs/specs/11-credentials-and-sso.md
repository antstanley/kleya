...
`kleya` does not manage AWS credentials itself. Every adapter call delegates to `aws-config`'s default credentials chain, which resolves to one of the supported sources at runtime: environment variables, an `~/.aws/config` profile, an IAM Identity Center (SSO) cached token, a `credential_process` helper, instance-profile metadata (IMDS), or an OIDC web-identity token. This page documents which sources are reachable in the current code, where each one is read from, how kleya's profile / region overrides interact with the SDK's own resolution, and how a credential failure surfaces.
...
| **SSO (cached token)** | `~/.aws/sso/cache/<sha1>.json` written by `aws sso login` | Run `aws sso login --profile <p>` once per ~8 h; kleya picks it up transparently |
| **Profile credential_process** | `~/.aws/config` `[profile <p>]` with `credential_process = <cmd>` | Configure a helper (e.g. `aws-vault exec`, `granted assume`) that prints JSON on stdout |
...
SSO **token consumption** works today, with no kleya-side code:
...
SSO **interactive login** (the device-authorization OIDC flow that writes the cache) is **deliberately out of scope for kleya**. The operator runs `aws sso login` (or `aws-vault`, `granted`, or any equivalent tool) before invoking kleya; kleya picks up the cached token transparently. See Decisions below.
...
| SSO token expired | Same as above, with `source` mentioning `ExpiredToken` or `RefreshFailure` | Re-run `aws sso login --profile <p>`; rerun the kleya command. |
